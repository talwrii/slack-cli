...
def run():
    parser = argparse.ArgumentParser(
        description="""Send, pipe, upload and
                                     receive Slack messages from the CLI"""
    )
    parser.add_argument(
        "-t",
        "--token",
        help="Explicitly specify Slack API token which will be saved to {}.".format(
            token.TOKEN_PATH
        ),
    )
    parser.add_argument(
        ...
        url: https://xxx.slack.com. Use this option to interact with different teams. If
    ...
    group_send.add_argument(
        "-d", "--dst", help="Send message to a Slack channel, group or username"
    ).completer = resource_completer
    group_send.add_argument("-f", "--file", help="Upload file")
    group_send.add_argument(
        "--pre", action="store_true", help="Send as verbatim `message`"
    )
    group_send.add_argument(
        "--run",
        action="store_true",
    ...
    for source in sources:
        for message in utils.get_messages(source, count=count):
            if json_format:
                ...
                print(utils.format_message(source_name, message))
